...
class ControlBusquedaMapas {
     ...
     */
    public function cargarSearchMapas($cont, $pkeyword) {
        //exit;
        $file = '../lib/.config';
        ...
        $sql = "SELECT t.titleno, t.title, fbibus_bibliografiaxtitulo (t.titleno) || ' -- [' || s.title || ']' bibliografia, o.locator imagen
                FROM titles t, titleseries ts, series s, titleobjs t_o, objects o
        ...
        while ($bd->siguienteFila($idConsulta)) {
            ...
            if ($imagen != null && $imagen != "null") {
                ...
                $mapa->setImagen($GLOBALS["app_route"].'/images/no_mapimage.jpg');
            }
            $elementos[] = $mapa;
        }
