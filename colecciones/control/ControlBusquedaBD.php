...
class ControlBusquedaBD {
     ...
     */
    public function cargarSearchBD($cont, $pBD, $pArea, $pTipoAcceso) {
        //exit;
        $file = '../lib/.config';
        ...
        $sql = "select distinct tb.titleno titleid, upper(tb.title) titulo, " .
                ...
                "where ('" . $pBD . "' is null or tb.titleno = '" . $pBD . "') and ('" . $pTipoAcceso . "' is null or fbibbus_accesos ( tb.titleno ) like '%'||'" . $pTipoAcceso . "'||'%') " .
                "and tb.titleno in (select distinct decode(t.articleno,null,t.titleno,(select t1.titleno from titles t1 where t1.titleno = t.articleno)) titleno " .
                "from titles t, copies c, classes cl, titleclasses tc where c.copycat = 'BD' " .
                "and cl.classtp = 'AREA' and ('" . $pArea . "' is null or cl.classmarkno = '" . $pArea . "') and tc.titleno = c.titleno " .
                "and tc.classmarkno = cl.classmarkno and tc.titleno = t.titleno)  order by 2 asc";
        /* $sql="select fbibbus_objeto ( tb.titleno, 'más información' ) ||'algo' masinfo from titles tb " .
          "where ('" . $pBD . "' is null or tb.titleno = '" . $pBD . "') and ('" . $pTipoAcceso . "' is null or fbibbus_accesos ( tb.titleno ) like '%'||'" . $pTipoAcceso . "'||'%') " .
          "and tb.titleno in (select distinct decode(t.articleno,null,t.titleno,(select t1.titleno from titles t1 where t1.titleno = t.articleno)) titleno " .
          "from titles t, copies c, classes cl, titleclasses tc where c.copycat = 'BD' " .
          "and cl.classtp = 'AREA' and ('" . $pArea . "' is null or cl.classmarkno = '" . $pArea . "') and tc.titleno = c.titleno " .
          "and tc.classmarkno = cl.classmarkno and tc.titleno = t.titleno)  order by 1 asc";
          echo 'SQL: '.$sql.'<br/>'; */
