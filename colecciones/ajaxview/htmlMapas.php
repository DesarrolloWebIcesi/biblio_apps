...
/* fin */
$controlBusqueda = $_SESSION['mapsearcher'];
if ($controlBusqueda == null || $controlBusqueda == '') {
    $controlBusqueda = new ControlBusquedaMapas();
    $_SESSION["mapsearcher"] = $controlBusqueda;
}

if ($newsearch == 's') {
    $controlBusqueda->cargarSearchMapas(0, $pkeyword);
}
...
/* fin */
echo HTMLResultadosMapas($tamano, $elementos, $pkeyword);
...
function HTMLResultadosMapas($tamano, $elementos, $keyword) {
    ...
    if ($tamano <= 0) {
        ...
        $html.= '<div class="subtitulo">Resultados de mapas ' . $mensaje . '</div>';
    }
