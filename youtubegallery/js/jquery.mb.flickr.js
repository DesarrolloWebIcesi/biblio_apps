 ...
 * ex:
 *
 jQuery("#myElementId").mb_loadFlickrPhotos({callBack: logData})
 function logData(o){
 ...
 jQuery(o.photos).each(function(n){
...
(function($) {
  ...
  jQuery.mbFlickr= {
    ...
    loadFlickrPhotos:function(options){
      ...
      gallery.defaults = {};
      jQuery.extend(gallery.defaults,jQuery.mbFlickr.defaults, options);
      ...
      if (!jQuery.mbFlickr.defaults.flickr_user_id)
        jQuery.mbFlickr.getFlickrNSID(jQuery.mbFlickr.flickr_api_key,jQuery.mbFlickr.flickr_user_name);
      if (getSet) {
        jQuery(gallery).mb_getFlickrSet(gallery.defaults.page,function(){
          jQuery(gallery).mb_getFlickrPhotoDATA();
          jQuery(gallery).mb_getFlickrPhotoINFO();
        });
      }else{
        jQuery(gallery).mb_getFlickrPhotos(gallery.defaults.page,function(){
          jQuery(gallery).mb_getFlickrPhotoDATA();
          jQuery(gallery).mb_getFlickrPhotoINFO();
        });
      }
    ...
    getFlickrNSID:function(key,name){
      jQuery.getJSON("http://api.flickr.com/services/rest/?method=flickr.urls.lookupUser&api_key="+key+"&url=http%3A%2F%2Fwww.flickr.com%2Fphotos%2F"+name+"%2F&format=json&rnd="+new Date()+"&jsoncallback=?",
              function(data){
                if (data.stat!="fail"){
                  jQuery.mbFlickr.defaults.flickr_user_id=data.user.id;
                }
              });
    },
    getFlickrSet:function(page, callBack){
      ...
      var key= jQuery.mbFlickr.flickr_api_key;
      var setID= gallery.defaults.flickr_photoset_id;
      jQuery.getJSON("http://api.flickr.com/services/rest/?method=flickr.photosets.getPhotos&api_key="+key+"&photoset_id="+setID+"&extras=url_sq,url_t,url_s,url_m,url_o&per_page="+per_page+"&page="+page+"&format=json&rnd="+new Date()+"&jsoncallback=?",
              function(data){
                if(data.stat=="fail") {
                  alert(data.message);
                  return;
    ...
    getFlickrPhotos:function(page, callBack){
      ...
      var key= jQuery.mbFlickr.flickr_api_key;
      var userNSID= jQuery.mbFlickr.defaults.flickr_user_id;

      jQuery.getJSON("http://api.flickr.com/services/rest/?method=flickr.people.getPublicPhotos&api_key="+key+"&user_id="+userNSID+"&per_page="+per_page+"&page="+page+"&format=json&jsoncallback=?",
              function(data){
    ...
    getFlickrPhotoDATA:function(){
      ...
      jQuery(gallery.photos).each(function(i){
        jQuery.getJSON("http://api.flickr.com/services/rest/?method=flickr.photos.getSizes&api_key="+key+"&photo_id="+this.id+"&secret="+this.secret+"&format=json&rnd="+new Date()+"&jsoncallback=?",
                function(data){
    ...
    getFlickrPhotoINFO:function(){
      ...
      jQuery(gallery.photos).each(function(i){
        ...
        jQuery.getJSON("http://api.flickr.com/services/rest/?method=flickr.photos.getInfo&api_key="+key+"&photo_id="+this.id+"&secret="+this.secret+"&format=json&rnd="+new Date()+"&jsoncallback=?",
                function(data){
  ...
  jQuery.fn.mb_getFlickrSet = jQuery.mbFlickr.getFlickrSet;
  jQuery.fn.mb_getFlickrPhotos = jQuery.mbFlickr.getFlickrPhotos;
  jQuery.fn.mb_getFlickrPhotoDATA = jQuery.mbFlickr.getFlickrPhotoDATA;
  jQuery.fn.mb_getFlickrPhotoINFO = jQuery.mbFlickr.getFlickrPhotoINFO;
  jQuery.fn.mb_loadFlickrPhotos = jQuery.mbFlickr.loadFlickrPhotos;
