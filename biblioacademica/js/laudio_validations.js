...
function titleSearcher(){
    ...
    jQuery("form#audioForm").validate({
        rules: {},
        messages: {},
        submitHandler:
        function(form) {
            //alert(jQuery("#pTipoAcceso").val().toString());
            loadBDResults(0, jQuery("form#audioForm #size").val(), jQuery("form#audioForm #newsearch").val(), jQuery("form#audioForm #pkeyword").val());
        }
    });
}
...
function loadBDResults(cont,size, newsearch,pkeyword){
    ...
    jQuery.ajax({
        type: "POST",
        url: coleccionesURL+"/ajaxview/htmlAudio.php",
        data: {
            cont:cont,
            size:size,
            newsearch:newsearch,
        ...
        success: function(list){
            ...
            jQuery.ajax({
                type: "POST",
                url: coleccionesURL+"/ajaxview/htmlAudioNav.php",
                data: {
                    cont:cont,
                    size:size,
                    newsearch:newsearch,
