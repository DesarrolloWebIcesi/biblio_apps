...
function titleSearcher(){
    ...
    jQuery("form#videosForm").validate({
        rules: {},
        messages: {},
        submitHandler:
        function(form) {
            //alert(jQuery("#pTipoAcceso").val().toString());
            loadBDResults(0, jQuery("form#videosForm #size").val(), jQuery("form#videosForm #newsearch").val(), jQuery("form#videosForm #pkeyword").val());
        }
    });
}
...
function loadBDResults(cont,size, newsearch,pkeyword){
    ...
    jQuery.ajax({
        type: "POST",
        url: coleccionesURL+"/ajaxview/htmlVideos.php",
        data: {
            cont:cont,
            size:size,
            newsearch:newsearch,
        ...
        success: function(list){
            ...
            jQuery.ajax({
                type: "POST",
                url: coleccionesURL+"/ajaxview/htmlVideosNav.php",
                data: {
                    cont:cont,
                    size:size,
                    newsearch:newsearch,
