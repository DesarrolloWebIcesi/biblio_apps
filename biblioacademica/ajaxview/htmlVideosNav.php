...
/**
 * Este archivo genera el HTML necesario para visualizar la paginación del listado de elementos
 * de la videoteca
 *
 * @author David Andrés Manzano -  damanzano
 * @since 15/10/10
 */
include_once '../control/ControlBusquedaVideos.php';
include_once ('../../commons/services/GeneralServices.php');
include_once ('../Config.php');

session_start();
$url_content = $GLOBALS["app_route"].'/ajaxview/htmlVideos.php';
$url_nav=$GLOBALS["app_route"].'/ajaxview/htmlVideosNav.php';;
$tab='videoteca';
...
$controlBusqueda = $_SESSION['bavideosearcher'];
if ($controlBusqueda == null || $controlBusqueda == '') {
    $controlBusqueda = new ControlBusquedaVideos();
    $_SESSION["bavideosearcher"] = $controlBusqueda;
}
...
echo HTMLNavegacionMapas($cont, $paginas, $size, $tamano, $url_content, $url_nav, $tab);
 ...
 */
function HTMLNavegacionMapas($cont, $paginas, $elemPaginas, $tamano, $url_content, $url_nav, $tab) {
    $html = '';
    if ($tamano <= 0) {
        $html = '-';
        return $html;
...
//    $html.= '</td>';
    ...
    if ($paginaactual > 1) {
        $style_inicio = 'class="app_navlink" title="Inicio" onclick="loadBDResults(\'0\',\'' . $elemPaginas . '\', \'n\',\'' . $url_content. '\',\'' . $url_nav. '\',\'' . $tab. '\');"';
        $style_anterior = 'class="app_navlink" title="Anterior" onclick="loadBDResults(\'' . ($cont - $elemPaginas) . '\',\'' . $elemPaginas . '\', \'n\',\'' . $url_content. '\',\'' . $url_nav. '\',\'' . $tab. '\');"';
        $tipo_imagen='';
    }
    ...
    for ($i = 0; $i < $paginas; $i++) {
        ...
        if (($numpag <= $paginas) && (($numpag >= ($paginaactual - 7 - $diferencia_final)) && ($numpag <= ($paginaactual + 7 + $diferencia_inicio)))) {
            $html.= '<td><a class="app_navlink' . $claseactual . '" onclick="loadBDResults(\'' . ($i * $elemPaginas) . '\',\'' . $elemPaginas . '\', \'n\',\'' . $url_content. '\',\'' . $url_nav. '\',\'' . $tab. '\');">' . $numpag . '</a></td>';
        }
    }
    $html.= '</tr></table>';
    $html.= '</td>';
    ...
    if ($paginaactual < $paginas) {
        $style_siguiente = 'class="app_navlink" title="Siguiente" onclick="loadBDResults(\'' . ($cont + $elemPaginas) . '\',\'' . $elemPaginas . '\', \'n\',\'' . $url_content. '\',\'' . $url_nav. '\',\'' . $tab. '\');"';
        $style_fin = 'class="app_navlink" title="Fin" onclick="loadBDResults(\''.($tamano-$modulo_pag).'\',\'' . $elemPaginas . '\', \'n\',\'' . $url_content. '\',\'' . $url_nav. '\',\'' . $tab. '\');"';
        $tipo_imagen_fin='';
    }
