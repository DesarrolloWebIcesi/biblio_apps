...
/**
 * Este archivo genera el HTML necesario para visualizar la paginación del listado de elementos multimedia
 *
...
session_start();
$url_content = $GLOBALS["app_route"].'/ajaxview/htmlMmedia.php';
$url_nav=$GLOBALS["app_route"].'/ajaxview/htmlMmediaNav.php';
$tab='mmedia';
...
$controlBusqueda = $_SESSION['bammediasearcher'];
if ($controlBusqueda == null || $controlBusqueda == '') {
    $controlBusqueda = new ControlBusquedaMmedia();
    $_SESSION["bammediasearcher"] = $controlBusqueda;
}
...
echo HTMLNavegacionMmedia($cont, $paginas, $size, $tamano, $url_content, $url_nav, $tab);
 ...
 */
function HTMLNavegacionMmedia($cont, $paginas, $elemPaginas, $tamano, $url_content, $url_nav, $tab) {
    $html = '';
    if ($tamano <= 0) {
        $html = '-';
        return $html;
